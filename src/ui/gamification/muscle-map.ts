...
// Professional body silhouette with polygon data
...
/**
 * Polygon data for anterior (front) view of the body
 * Each muscle group has one or more polygons defined by space-separated coordinates
 */
const ANTERIOR_POLYGONS = {
  // Chest (pecho)
  chest: [
...
/**
 * Polygon data for posterior (back) view of the body
 */
const POSTERIOR_POLYGONS = {
  // Head
  head: [
...
/**
 * Maps muscle groups to our gamification system
 */
const MUSCLE_GROUP_MAPPING: Record<GamificationMuscleGroup, {
  anterior: (keyof typeof ANTERIOR_POLYGONS)[];
  posterior: (keyof typeof POSTERIOR_POLYGONS)[];
}> = {
  pecho: { anterior: ['chest'], posterior: [] },
  espalda: { anterior: [], posterior: ['trapezius', 'upperBack', 'lowerBack'] },
  hombros: { anterior: ['frontDeltoids'], posterior: ['backDeltoids'] },
  biceps: { anterior: ['biceps'], posterior: [] },
  triceps: { anterior: ['triceps'], posterior: ['triceps'] },
  core: { anterior: ['abs', 'obliques'], posterior: [] },
  gluteos: { anterior: [], posterior: ['gluteal'] },
  piernas: { anterior: ['quadriceps', 'calves', 'abductors'], posterior: ['hamstring', 'calves', 'abductor'] },
};

/**
 * Creates polygon elements from point strings
 */
...
/**
 * Generates the glow filter for SVG
 */
function generateGlowFilter(id: string, glowColor: string): string {
  return `
    <filter id="${id}" x="-50%" y="-50%" width="200%" height="200%">
      <feDropShadow dx="0" dy="0" stdDeviation="2" flood-color="${glowColor}" flood-opacity="0.8"/>
    </filter>
  `;
}

/**
 * Renders the full muscle map with colors based on ranks
 * Shows FRONT view only for a cleaner look
 */
export function renderMuscleMap(
  ...
  const muscles: GamificationMuscleGroup[] = [
    ...
    'triceps', 'core', 'gluteos', 'piernas'
  ];

  // Collect all glow filters needed
  const glowFilters: string[] = [];

  // Generate colored muscle polygons for ANTERIOR view
  const musclePolygons = muscles.map(muscle => {
    ...
    if (hasGlow) {
      glowFilters.push(generateGlowFilter(filterId, colors.glow));
    }

    const mapping = MUSCLE_GROUP_MAPPING[muscle];
    let result = '';

    // Anterior polygons
    for (const key of mapping.anterior) {
      const polygons = ANTERIOR_POLYGONS[key];
      if (polygons) {
        result += createPolygons(polygons, colors.fill, hasGlow, filterId, muscle);
      }
    }
  ...
  // Base body parts (not tracked) - head, neck, forearm, knees
  const baseBodyParts = `
    ${createPolygons(ANTERIOR_POLYGONS.head, '#4B5563', false, '', 'head')}
    ${createPolygons(ANTERIOR_POLYGONS.neck, '#4B5563', false, '', 'neck')}
    ${createPolygons(ANTERIOR_POLYGONS.forearm, '#4B5563', false, '', 'forearm')}
    ${createPolygons(ANTERIOR_POLYGONS.knees, '#374151', false, '', 'knees')}
  `;

  return `
    <svg viewBox="0 0 100 200" width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg" class="muscle-map">
      <defs>
        ${glowFilters.join('\n')}
      </defs>

      <!-- Base body outline (hands, feet) -->
      <ellipse cx="5" cy="105" rx="5" ry="8" fill="#374151"/>
      <ellipse cx="95" cy="105" rx="5" ry="8" fill="#374151"/>
      <ellipse cx="23" cy="200" rx="6" ry="4" fill="#374151"/>
      <ellipse cx="77" cy="200" rx="6" ry="4" fill="#374151"/>
...
/**
 * Renders a compact muscle map showing BOTH front and back views
 */
export function renderMuscleMapDual(
  muscleRanks: MuscleRanks,
  width: number = 200,
  height: number = 200
): string {
  ...
  // Generate colored muscle polygons for BOTH views
  const anteriorPolygons = muscles.map(muscle => {
    const rankData = muscleRanks[muscle];
    const colors = RANK_COLORS[rankData.rank];
    const hasGlow = colors.glow !== 'none';
    const filterId = `glow-a-${muscle}`;

    if (hasGlow && !glowFilters.some(f => f.includes(filterId))) {
      glowFilters.push(generateGlowFilter(filterId, colors.glow));
    }
  ...
  const posteriorPolygons = muscles.map(muscle => {
    const rankData = muscleRanks[muscle];
    const colors = RANK_COLORS[rankData.rank];
    const hasGlow = colors.glow !== 'none';
    const filterId = `glow-p-${muscle}`;
    ...
    for (const key of mapping.posterior) {
      const polygons = POSTERIOR_POLYGONS[key as keyof typeof POSTERIOR_POLYGONS];
      if (polygons) {
        result += createPolygons(polygons, colors.fill, hasGlow, filterId, muscle);
      }
    }
  ...
  const posteriorBase = `
    ${createPolygons(POSTERIOR_POLYGONS.head, '#4B5563', false, '', 'head')}
    ${createPolygons(POSTERIOR_POLYGONS.forearm, '#4B5563', false, '', 'forearm')}
    ${createPolygons(POSTERIOR_POLYGONS.knees, '#374151', false, '', 'knees')}
  `;

  return `
    <svg viewBox="0 0 210 200" width="${width}" height="${height}" xmlns="http://www.w3.org/2000/svg" class="muscle-map-dual">
      <defs>
        ${glowFilters.join('\n')}
      </defs>
      ...
      <g transform="translate(110, 0)">
        <!-- Hands & feet -->
        <ellipse cx="5" cy="108" rx="5" ry="8" fill="#374151"/>
        <ellipse cx="95" cy="108" rx="5" ry="8" fill="#374151"/>
        <ellipse cx="27" cy="202" rx="6" ry="4" fill="#374151"/>
        <ellipse cx="70" cy="202" rx="6" ry="4" fill="#374151"/>
        ${posteriorBase}
        ${posteriorPolygons}
      </g>
    </svg>
...
/**
 * Renders a mini muscle map for header/hero
 */
export function renderMuscleMapMini(muscleRanks: MuscleRanks): string {
  return `
    <div class="w-20 h-32 flex-shrink-0">
      ${renderMuscleMap(muscleRanks, 80, 160)}
    </div>
  `;
}

/**
 * Renders the muscle map with a legend showing all muscle ranks
 */
  ...
  return `
    <div class="flex gap-4 items-start">
      <div class="flex-shrink-0">
        ${renderMuscleMap(muscleRanks, 120, 240)}
      </div>
...
/**
 * Renders a single muscle progress indicator
 */
